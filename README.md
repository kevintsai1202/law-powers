...
> ## 📜 使用授權
>
> 本專案**無償授權所有人**自由使用 —— 唯 **經兆國際法律事務所** 除外。
> 拜他們所賜，才有這個技能包的誕生。
...
| `lawbank-query-builder` | 建議（貼回之法條／判決交叉驗證） | 不需要 |
...
flowchart TD
    ...
    LQB["lawbank-query-builder<br/>法源布林檢索式產生・人工檢索轉介"]
    EXT[("法源法律網（付費會員）<br/>使用者本人登入檢索")]
...
*   **`lawbank-query-builder`**：法源法律網（lawbank.com.tw）檢索式產生器——涉及行政函釋、法學論著、修法沿革等官方免費庫未涵蓋之資料時，將需求轉譯為法源布林檢索式（＆且、＋或、－不含）與分區入口，由**使用者本人**登入人工檢索；貼回結果依防幻覺紀律驗證標註後方得引用。技能鐵律：不自動連線法源、不索取或儲存帳號密碼、不將其內容建庫（遵守法源會員條款對程式化擷取之禁令）。
...
*   **`legal-graph`**：將案情、法條、判決、爭點、當事人、證據，以及 `contract → clause → obligation` 契約義務三層模型與風險評級彙整為 superset 法律關係圖資料；隨附自包含 3D 渲染器（`skills/legal-graph/renderer/`）可直接檢視。案件有期限管理需求時可加掛「程序時序」儀表板分頁（期限倒數、等比例時間軸、程序路線對照）；另可產出自包含 SVG「訴訟決策樹＋因果關係徑路圖」單檔雙面板圖（決策分支錨定條號判決、因果評價轉錄涵攝表），供書狀附圖與列印。
...
5. **互動檢視**：拖曳旋轉、滾輪縮放；點擊節點開啟詳情面板（判決勝敗徽章、條款風險、要件該當性）；對照圖例判讀節點與連線顏色；以節點類型篩選器顯示／隱藏特定類型；同 `family` 標籤之案件叢集可聚焦檢視。若 `data.js` 含 `timeline` 欄位，頁面右上會出現「程序時序」分頁——期限倒數、等比例時間軸與程序路線對照表，日期參數可直接調整並自動重算，列印時自動轉淺色版面供附卷。
...
> 「幫我畫出這個案子的訴訟決策樹和因果關係徑路圖，並整理上訴、告訴與時效期限。」

1. `legal-graph` 步驟五以自包含 SVG 產出上下雙面板圖：上面板為訴訟決策樹（若…則…分支，每個判斷錨定已查證條號或判決字號，未查證者虛線框標示）；下面板為「加害行為 →（介入因素）→ 損害結果」因果關係徑路圖，每段因果評價（○✗△）一律轉錄 `legal-element-analysis` 涵攝表，不得臆測。
2. `legal-graph` 步驟四於 `data.js` 加入 `timeline` 欄位，渲染頁自動出現「程序時序」分頁：期限倒數、等比例時間軸與程序路線對照表；期限依據（條號、期間、起算）先經 `legal-research` 查證，起算日為可調參數。
...
- `legal-research` 路由表新增函釋／論著／修法沿革轉介列；步驟四信任閘門 fail-closed 範本加入法源人工擴大檢索選項。

### 2026-08-03
- `legal-graph` 新增步驟五：以自包含 SVG 產出「訴訟決策樹＋因果關係徑路圖」單檔雙面板圖——決策分支錨定已查證條號／判決（未查證者虛線框）、因果評價（○✗△）一律轉錄涵攝表；隨附繪製規範 `references/svg-diagrams.md` 與驗證腳本 `scripts/verify-svg-diagrams.mjs`。
- `legal-graph` 新增步驟四：程序時序儀表板——`data.js` 加入 `timeline` 選用欄位後，渲染頁自動出現「程序時序」分頁（期限倒數、等比例時間軸、程序路線對照表；起算日可調並自動重算，列印自動轉淺色）；schema 詳 `references/timeline-schema.md`。
- `legal-brainstorming` 選單④擴充為視覺化圖表雙選項（3D 關係圖／SVG 決策樹＋因果圖）；`legal-element-analysis` 新增因果徑路圖轉介。
- `legal-research` 觸發說明改寫為「台灣法律問題的強制入口」，明確列出裁判費、時效等數值型觸發語，強化授權子法反查不可略過之定位。
