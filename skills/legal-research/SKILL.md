...
*   **規則**：Agent 在啟動或接收到法律查詢時，必須先確認是否能調用 `taiwan-legal-db`（法規／釋字／判決結構化檢索，**必要**）與 `dr-lawbot`（判例語意檢索，預設引擎；未安裝時可降級）兩組 MCP 工具。
*   **引導機制**：
...
*   **規則**：依查詢意圖選擇工具，預設「判例找論理 → 語意檢索優先」。

#### 路由決策
| 需求 | 使用工具 |
|---|---|
| 從案情找相關判例、論理（**預設入口**） | `dr-lawbot:search_bundle`（`search_type=hybrid`、`read_top=3`） |
| 已知字號，或依法院／年度／案類／輸贏方精確過濾 | `taiwan-legal-db:search_judgments`（`case_word`+`case_number`、或 `main_text`） |
...
#### 語意檢索（預設路徑）
*   將案情以自然語言直接交給 `dr-lawbot:search_bundle`，取回 `twlegalrag.bundle/v1`。
...
#### 關鍵字檢索（精確定位路徑）
*   使用 `taiwan-legal-db:search_judgments` 時，勿直接用口語，須先轉為台灣法律用語：
...
#### 優雅降級
*   若環境未配置 `dr-lawbot`，判例檢索改用 `taiwan-legal-db:search_judgments` 關鍵字檢索，並**一次性**告知：「語意檢索未啟用，目前以關鍵字檢索；安裝 dr-lawbot 可提升相關判例檢索的涵蓋。」不 fail-closed。
