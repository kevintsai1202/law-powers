...
- 憑空杜撰公文所引用之法規或案號——法源查證仍應依 `legal-research` 與 `.agents/AGENTS.md` 第 1 點之檢索優先原則辦理，本技能只規範「格式與用語」，不取代法源查證。
...
*   **動作**：先確認使用者要撰寫的是「函」「簽」「公告」或其他公文（書函、開會通知單等），並判斷行文對象是：
    ...
    *   **下行文**：對所屬下級機關或人民（如：台端）。
*   **依據**：《公文程式條例》第 2 條之公文種類分類（令、咨、函、公告、其他公文）。
...
*   **法源提醒**：若公文內容引用特定法規或案號，其正確性仍受 `.agents/AGENTS.md` 第 1 點「檢索優先與防幻覺原則」拘束——本技能不得自行杜撰法條或字號，如需查證應提示使用者另行呼叫 `legal-research`。
*   **免責聲明**：若公文內容涉及法律見解、權利義務判斷或訴訟／爭議策略（例如：申覆決定書、法律異議函），應依 `.agents/AGENTS.md` 第 4 點附加免責聲明；純屬行政協調性質之公文（如會議通知、一般知會函）則不需要。
