---
name: legal-graph
description: 引導 Agent 彙整案情事實、法條、判決、爭點、當事人、證據、契約義務關係（契約→條款→義務三層模型，含風險評級上色）與構成要件涵攝結果（法條→要件→事實該當性上色），產出與 law-powers 之 index.html/data.js 相容的 superset 法律關係圖譜資料，並寫入 data.js。另可依需求以自包含 SVG 產出「訴訟決策樹＋因果關係徑路圖」單檔雙面板圖（決策分支錨定條號判決、因果評價轉錄涵攝表），供書狀附圖與列印。
---
...
graph TD
    ...
    Step4["步驟四（選用）：程序時序資料 (timeline)"]
    Step5["步驟五（選用）：SVG 訴訟決策樹＋因果徑路圖"]
...
> **僅需 SVG 圖時可獨立執行步驟五**：使用者只要求決策樹或因果徑路圖、且無建置關係圖譜需求時，得跳過步驟一至四直接執行步驟五（資料紀律不變）。
...
### 步驟四（選用）：程序時序資料 (timeline)

> **完整 schema 與欄位定義**：請參閱 [references/timeline-schema.md](references/timeline-schema.md)。本節僅規範何時建構與資料紀律。

當案件有**程序期限管理需求**（上訴、告訴、時效、提存等倒數），或使用者要求「時序圖／期限儀表板」時，於 `window.GRAPH_DATA` 加入選用之 `timeline` 欄位。渲染頁會自動出現「關係圖｜程序時序」分頁；未加入此欄位時分頁隱藏，不影響既有圖譜（向後相容）。

建構紀律（違反任一項即不得產出）：

1. **期限依據必經查證**：`deadlines[].basis` 之條號、期間長度、起算規定，一律先以 `legal-research` 雙軌檢索查證，不得憑記憶撰寫（agents-rules §1）。
2. **事實分類必須正確**：`events[].cls` 之 `""`（判決認定）／`"alleged"`（當事人陳述，法院未認定）／`"judgment"`（宣判）為法律語意標記，判決理由「縱使有之」等未認定事實一律標 `alleged`。
3. **起算日開放調整**：期限起算繫於送達日、知悉日等個案事實，應定義為 `params` 輸入欄，`hint` 註明預設值性質；在途期間、假日順延等未計入之限制寫入 `warn`／`note`。
4. **跨度懸殊之期限**標 `offAxis: true`（不入等比例軸、右側箭頭註記），避免壓縮其他節點。
5. **事件與圖譜節點同源**：timeline 之 events 應與圖譜 `fact`／`judgment` 節點取自同一份事實整理，日期、名稱不得兩處矛盾。

完成提示語（附加於步驟三提示語之後）：
    > 「本案已一併產出程序時序資料：開啟頁面後點右上『程序時序』分頁即可檢視期限倒數、等比例時間軸與程序路線對照表；日期參數可直接調整並自動重算。列印該分頁會自動轉為淺色版面供附卷使用。」

---

### 步驟五（選用）：SVG 訴訟決策樹＋因果關係徑路圖

> **完整繪製規範**：節點形狀語彙、配色、佈局、範本骨架與資料紀律，請參閱 [references/svg-diagrams.md](references/svg-diagrams.md)。本節僅規範觸發時機與資料來源。

當使用者要求「決策樹／訴訟流程圖／因果關係圖／歸責路徑圖」，或經 `legal-brainstorming`（策略報告完成後）／`legal-element-analysis`（涵攝表含因果類要件）轉介時，以**單一自包含 SVG 檔（上下雙面板）**產出：

*   **上面板（訴訟決策樹）**：資料來自 `legal-brainstorming` 步驟三、四之爭點攻防與策略報告；每個判斷分支之法效錨定已查證條文或 `allowed_citations` 內判決，未查證者虛線框標「（待查證）」。
*   **下面板（因果關係徑路圖）**：呈現「加害行為 →（介入因素）→ 損害結果」歸責鏈；每段因果評價（○✗△）**一律轉錄 `legal-element-analysis` 涵攝表，不得臆測**——與 `element` 節點 `met` 之紀律相同；尚未涵攝時全部標 △ 並註明。
*   僅有其中一種資料時退化為單面板；輸出至目前案件工作目錄 `outputs/decision-causal-<案件代稱>.svg`，**不寫入 `data.js`**。本圖與 3D 圖譜互補——關係圖呈全貌網絡，本圖呈線性敘事，適合書狀附圖與列印。
*   完成提示語依 references/svg-diagrams.md 第七節。
